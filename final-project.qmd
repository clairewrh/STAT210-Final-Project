...
Before examining the parameters of interest, we noticed that 173 out of the 919 observations have 0 mm/dl for their serum cholesterol level. 0 mm/dl serum cholesterol is physiologically impossible, hence we believe that these observations are missing cholesterol data, and the missing mechanism is unknown. We decide to use complete case analysis because it is relatively easy to interpret, and it can reduce bias in the estimates of the variables of interest when the missing data are missing completely at random or missing at random.

```{r mutate cholesterol}
...
```{r box plot for age }
ggplot(heart, aes(x = Age, y = HeartDisease, group = HeartDisease)) + 
  geom_boxplot() +
...
ggplot(heart, aes(x = Cholesterol, y = HeartDisease, group = HeartDisease)) +  
  geom_boxplot() +
...
ggplot(heart, aes(x = RestingBP, y = HeartDisease, group = HeartDisease)) + 
  geom_boxplot() +
...
ggplot(heart, aes(x = MaxHR, y = HeartDisease, group = HeartDisease)) +  
  geom_boxplot() +
...
x <- model.matrix(HeartDisease ~ ., data = heart)
...
```{r stepwise}
m_none <- lm(HeartDisease ~ 1, data = heart)
